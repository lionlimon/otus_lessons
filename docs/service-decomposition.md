...
**Риск 3. Прямой доступ к чужим данным — нарушение владения.**

Тикетница для маршрутизации нуждается в ролях пользователей. Соблазн — читать таблицу ролей напрямую из БД администрирования, минуя его API. Это классический data-coupling: тикетница знает внутреннюю схему чужого сервиса, любое изменение структуры БД администрирования ломает тикетницу.

Снимается **контрактом**: каждый сервис открывает только публичный API; прямого доступа к чужим БД нет. Тикетница запрашивает роли через API администрирования — и зависит только от формата ответа, а не от внутренней схемы хранения.
